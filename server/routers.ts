...
export function rankMirrors(mirrors: Mirror[]): Mirror[] { return mirrors.filter(item => item.published !== false && item.down !== true && item.api !== false && (item.uptime ?? 0) >= 90).sort((a, b) => (b.uptime ?? 0) - (a.uptime ?? 0)); }
...
async function chooseMirror(manual?: string): Promise<{ mirror: Mirror; candidates: Mirror[] }> {
  ...
  if (manual) { const uri = clean(manual); const found = discovered.find(item => item.uri === uri) ?? { name: new URL(uri).hostname, uri }; const checked = await checkMirror(found); if (checked.healthy) return { mirror: checked, candidates: [checked, ...discovered.filter(item => item.uri !== checked.uri)] }; }
...
export function normalizeSearchResults(items: unknown[]): unknown[] { return items.filter(item => item && typeof item === "object").map(item => { const value = { ...(item as Record<string, unknown>) }; if (!value.type) value.type = value.playlistId ? "playlist" : value.authorId && !value.videoId ? "channel" : "video"; if (value.type === "channel" && !value.title && value.author) value.title = value.author; if (value.type === "playlist" && !value.title && value.playlistTitle) value.title = value.playlistTitle; return value; }); }

export async function searchInvidious(query: string, currentPage?: number, type?: "video" | "channel" | "playlist" | "all", manual?: string) { try { return normalizeSearchResults(await invidious<unknown[]>("/search", { q: query, page: currentPage, type }, manual)); } catch (error) { if (type === "video") return normalizeSearchResults(await invidious<unknown[]>("/search", { q: query, page: currentPage }, manual)); throw error; } }
...
export const appRouter = router({
  ...
  invidious: router({
    ...
    search: publicProcedure.input(z.object({ query: z.string().trim().min(1).max(120), page, type: z.enum(["video", "channel", "playlist", "all"]).default("video"), mirror: selectedMirror.optional() })).query(({ input }) => searchInvidious(input.query, input.page, input.type, input.mirror)),
