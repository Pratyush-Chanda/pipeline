...
GitHub Pages hosts the static interface but cannot run this project’s Express/tRPC server. Native API-powered feeds, search, metadata, and direct media therefore require a separate server deployment. For a build-time connection, set `VITE_API_ORIGIN` to that server’s HTTPS origin; the client will then call `${VITE_API_ORIGIN}/api/trpc` instead of the Pages domain. Alternatively, open pipeline’s mirror settings on the published site and enter the server origin under **Server connection**; it is stored locally and applied after reload. Use Pages for the static shell and the project hosting flow for the full server-backed application.
